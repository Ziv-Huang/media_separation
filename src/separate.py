...
class Separation:
    ...
    def fill_black_width(self, frame, vacant_width: int):
        left = right = int(vacant_width / 2)
        frame = cv2.copyMakeBorder(
            frame, 0, 0, left, right, cv2.BORDER_CONSTANT, value=(0, 0, 0)
        )
        return frame

    def fill_black_bottom(self, frame, vacant_bottom: int):
        frame = cv2.copyMakeBorder(
            frame, 0, vacant_bottom, 0, 0, cv2.BORDER_CONSTANT, value=(0, 0, 0)
        )
        return frame

    def video_banner(self, video_name: str, show: bool = False):
        ...
        writer = cv2.VideoWriter(
            os.path.join(self.output, "output_" + os.path.basename(video_name)),
            cv2.VideoWriter_fourcc(*"H264"),
            fps,
            (width, self.banner_height),
        )
        ...
        with Bar(
            ...
            while cap.isOpened():
                ...
                bar.next()
                if ret:
                    banner_frame = self.fill_black_bottom(
                        frame, self.banner_height - height
                    )
                    writer.write(banner_frame)
                    if show:
                        cv2.imshow(video_name, banner_frame)
                        cv2.waitKey(1)
                else:
                    break
    ...
    def video_change_resolution(self, video_name: str, resolution, show: bool = False):
        cap = cv2.VideoCapture(video_name)
        width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        frame_number = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        ...
        writer = cv2.VideoWriter(
            os.path.join(self.output, os.path.basename(video_name)),
            cv2.VideoWriter_fourcc(*"H264"),
            fps,
    ...
    def video_separation(self, video_name: str, show: bool = False):
        ...
        writer_left = cv2.VideoWriter(
            os.path.join(self.output, video_left),
            cv2.VideoWriter_fourcc(*"H264"),
            fps,
            (int(width / self.parts_number), height),
        )
        writer_middle = cv2.VideoWriter(
            os.path.join(self.output, video_middle),
            cv2.VideoWriter_fourcc(*"H264"),
            fps,
            (int(width / self.parts_number), height),
        )
        writer_right = cv2.VideoWriter(
            os.path.join(self.output, video_right),
            cv2.VideoWriter_fourcc(*"H264"),
            fps,
            (int(width / self.parts_number), height),
        )
            ...
            while cap.isOpened():
                ...
                if ret:
                    if width / height <= ratio:
                        frame = self.fill_black_width(frame, vacant_width)
                    frame_left = frame[:, 0 : int(width / self.parts_number)]
                    frame_middle = frame[
                        :,
                        int(width / self.parts_number) : int(width / self.parts_number)
                        * 2,
                    ]
    ...
    def image_separation(self, image_name: str, show: bool = False):
        ...
        with Bar(
            "Separating", fill="█", suffix="%(percent).1f%% - %(eta)ds", max=1
        ) as bar:
            bar.next()
        ...
        frame_left = frame[:, 0 : int(width / self.parts_number)]
        frame_middle = frame[
            :, int(width / self.parts_number) : int(width / self.parts_number) * 2
        ]
        frame_right = frame[:, int(width / self.parts_number) * 2 : width]
    ...
    def separation(self, args: any):
        ...
        if args.video:
            ...
            self.video_change_resolution(
                args.change, (args.width, args.height), args.show
            )
        elif args.banner:
            self.video_banner(args.banner, args.show)
